...
async def test_modules_basic_checks(events, httpx_mock):
    ...
    for module_name, preloaded in all_preloaded.items():
        ...
        assert all(
            type(t) == str for t in watched_events), f"{module_name}.watched_events entries must be of type string"
        assert all(
            type(t) == str for t in produced_events), f"{module_name}.produced_events entries must be of type string"
